...
test('User can see other users new articles in "Your Feed" after following their profile', async ({
  ...
  await expect(homePage.articleTitleLocator(articleWithoutTags.title).nth(1)).toBeVisible(); 
  await expect(homePage.articleTitleLocator(articleWithoutTags.description).nth(1)).toBeVisible(); 
 await expect(homePage.page.getByRole('link', { name: user1.username }).nth(1)).toBeVisible();
});